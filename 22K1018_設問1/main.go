...
func init() {
	...
	//課題のため追加
	flag.StringVar(&flagOutputFile, "file", ".", "出力するファイル名(拡張子「.csv」はいらない)")
}

func main() {
	flag.Parse()
	CheckFilename(flagOutputFile) //課題のために追加　ファイル名の末尾に.csvがついているかチェック
	...
	for {
		...
		case 3: // 集計
			...
			file := filepath.Join(flagOutputDir, flagOutputFile+".csv")
