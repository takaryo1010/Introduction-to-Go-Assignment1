...
func CheckFilename(inputName string) {
	fmt.Scan(inputName)
	checktext := strings.Split(inputName, ".")
	if checktext[len(checktext)-1] == "csv" {
		fmt.Fprintln(os.Stderr, "エラー：", "指定されたファイル名の末尾に.csvが入っています")
		os.Exit(1)
	}
